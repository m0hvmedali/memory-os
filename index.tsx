...
const AnalyticsDashboard = ({ memories, vectorStore, onInitVectors, isLoadingVectors }: AnalyticsProps) => {
    ...
    const performSemanticSearch = async () => {
        ...
        try {
            ...
            // Map chunks back to messages (if possible) or just text analysis
            // Since our chunks store raw messages in meta.msgs (if we add it) or we just parse text
            // For now, let's treat the 'text' of the chunks as the messages to analyze
            const relevantMessages = topK.map(k => ({
                message: k.text,
                sender: k.meta.sender || 'Unknown', // We need to ensure meta has sender info or infer it
                timestamp: k.meta.timestamp
            }));
            
            setSemanticResults(relevantMessages);
        } catch (e) {
    ...
    }, [conversations, selectedId, semanticResults]);
    ...
    return (
        <div className="p-4 space-y-6 pb-24 overflow-y-auto h-full custom-scrollbar">
             ...
             <div className="flex flex-col gap-4">
                ...
                {/* Semantic Filter Bar */}
                <div className="bg-zinc-900/50 p-2 rounded-xl border border-zinc-800 flex gap-2">
                    {vectorStore.length > 0 ? (
                         <div className="flex-1 relative">
                            ...
                            <input 
                                ...
                                placeholder="Semantic Filter: e.g. 'work stress', 'happy memories'..."
                                className="w-full bg-zinc-950 rounded-lg pl-9 pr-8 py-2 text-sm focus:ring-1 focus:ring-indigo-500 outline-none"
                            />
                            {semanticResults.length > 0 && (
                                <button 
                        ...
                        <button 
                            ...
                            className="flex-1 flex items-center justify-center gap-2 bg-indigo-500/10 text-indigo-400 py-2 rounded-lg text-sm border border-indigo-500/20 hover:bg-indigo-500/20 transition-all"
                        >
                            {isLoadingVectors ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
                            Enable Semantic Analytics (Local AI)
                        </button>
                    )}
                    {vectorStore.length > 0 && (
                        <button 
                ...
                {semanticResults.length > 0 && (
                    <div className="bg-indigo-500/10 border border-indigo-500/20 px-3 py-2 rounded-lg text-xs text-indigo-300 flex items-center gap-2">
                        <Sparkles className="w-3 h-3" />
                        Viewing stats for {semanticResults.length} semantic matches relating to "{semanticQuery}"
                    </div>
                )}
            </div>
            ...
            {semanticResults.length > 0 && (
                <div className="bg-zinc-900/30 p-4 rounded-xl border border-zinc-800/50">
                    ...
                    <div className="space-y-3">
                        {semanticResults.slice(0, 5).map((m, i) => (
                            <div key={i} className="bg-zinc-950 p-3 rounded-lg text-sm border border-zinc-800">
                                ...
                                <div className="text-zinc-300 leading-relaxed">{m.message}</div>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-zinc-950 text-zinc-200 font-sans selection:bg-indigo-500/30">
      ...
      `}>
        <div className="p-6 border-b border-zinc-900">
            ...
            <p className="text-xs text-zinc-500 mt-1">v0.9.3 • Neural Analyst</p>
        </div>

        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
            <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-wider mb-2 px-3 mt-2">Data Layer</div>
            <NavItem icon={UploadCloud} label="Import" active={activeView === 'upload'} onClick={() => setActiveView('upload')} />
            <NavItem icon={Database} label="Memories" active={activeView === 'memories'} onClick={() => setActiveView('memories')} />
            
            <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-wider mb-2 px-3 mt-6">Intelligence</div>
            <NavItem icon={Brain} label="Neural Analyst" active={activeView === 'intelligence'} onClick={() => setActiveView('intelligence')} />
            <NavItem icon={BarChart2} label="Deep Analytics" active={activeView === 'analytics'} onClick={() => setActiveView('analytics')} />
            
            <div className="text-[10px] font-bold text-zinc-600 uppercase tracking-wider mb-2 px-3 mt-6">Write</div>
            <NavItem icon={PenTool} label="Journal" active={activeView === 'journal'} onClick={() => setActiveView('journal')} />
        </nav>

        <div className="p-4 border-t border-zinc-900">
            <NavItem icon={Settings} label="System" active={activeView === 'settings'} onClick={() => setActiveView('settings')} />
        </div>
      </aside>

      {/* Main Content */}
      <main className="flex-1 h-full w-full relative overflow-hidden bg-zinc-950" onClick={() => window.innerWidth < 768 && setIsSidebarOpen(false)}>
         {renderContent()}
         
         {/* Toast Container */}
         <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
             {toasts.map(toast => (
                 <div key={toast.id} className="pointer-events-auto bg-zinc-900 border border-zinc-800 text-white px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-slide-up min-w-[300px]">
                     {toast.type === 'success' && <CheckCircle className="w-5 h-5 text-emerald-500" />}
                     {toast.type === 'error' && <AlertCircle className="w-5 h-5 text-red-500" />}
                     {toast.type === 'info' && <Info className="w-5 h-5 text-blue-500" />}
                     <div>
                         <div className="text-sm font-medium">{toast.title}</div>
                         <div className="text-xs text-zinc-400">{toast.message}</div>
                     </div>
                 </div>
             ))}
         </div>
...
const NavItem = ({ icon: Icon, label, active, onClick }: any) => (
    <button 
        onClick={onClick}
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all group ${active ? 'bg-indigo-500/10 text-indigo-400' : 'text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200'}`}
    >
        <Icon className={`w-4 h-4 transition-colors ${active ? 'text-indigo-400' : 'text-zinc-500 group-hover:text-zinc-300'}`} />
        <span>{label}</span>
